-- 0004_referrer_and_device.sql
-- Two changes:
--   1. Track which school referred each signup (signups.referrer_school_id).
--      Frontend appends ?ref=<slug> to share URLs; backend resolves slug → id.
--   2. Block re-signups from the same device, even in incognito with a
--      different phone (signups.device_id, UNIQUE). Frontend computes a
--      browser fingerprint via FingerprintJS open source. Determined users
--      can still bypass with Tor / aggressive privacy browsers — that's
--      the cost of doing this client-side without IP/CAPTCHA — but it
--      stops casual incognito-and-resubmit.
--
-- Also: register_signup + get_progress now return school_slug so the
-- frontend can build referral URLs for any school (canonical or custom).

ALTER TABLE signups
  ADD COLUMN IF NOT EXISTS device_id TEXT,
  ADD COLUMN IF NOT EXISTS referrer_school_id UUID REFERENCES schools(id) ON DELETE SET NULL;

CREATE UNIQUE INDEX IF NOT EXISTS signups_device_id_unique ON signups(device_id) WHERE device_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS signups_referrer_school_id_idx ON signups(referrer_school_id);
...
BEGIN
  ...
  -- Device duplicate (same device, different phone — incognito bypass attempt).
  ...
  -- Resolve referrer slug → school_id (silently ignored if not a known slug).
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Either phone or device_id collision (race). Return duplicate with whichever existing row matches.
    SELECT s.id   AS school_id,
           s.slug AS school_slug,
           s.name AS school_name,
           s.signup_count,
