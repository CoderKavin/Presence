-- 0003_substring_collapse.sql
-- Tighten auto-collapse: also collapse when one normalized name is a substring
-- of the other AND both are reasonably long (≥ 8 chars) AND trigram similarity
-- is at least 0.55. This catches cases like
--   "trivandrum international"  →  "trivandrum international school"
-- that fall below the 0.85 threshold but are clearly the same school.
-- The length guard prevents short tokens like "Brown" or "USC" from
-- accidentally collapsing into longer-named institutions that happen to
...
BEGIN
  ...
  ELSE
    ...
    -- 2. Auto-collapse: high similarity OR substring with both names long enough.
    IF v_school_id IS NULL THEN
      SELECT s.id, s.name
